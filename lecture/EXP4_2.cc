...
protected:
    ...
    string post;
public:
    Cadre(string n, string a, string s, 
...
Teacher::Teacher(string n, string a, string s, 
    ...
    sex(std::move(s)), title(std::move(t)), 
    address(std::move(addr)), phone(std::move(tel)) {}
...
Cadre::Cadre(string n, string a, string s, 
    string p, string addr, string tel)
    : name(std::move(n)), age(std::move(a)), 
    sex(std::move(s)), post(std::move(p)), 
    address(std::move(addr)), phone(std::move(tel)) {}
// class Teacher_Cadre
Teacher_Cadre::Teacher_Cadre(string n, string a, string s, 
    ...
    Cadre(n, a, s, p, addr, tel), wages(w) {}
...
int main() {
    ...
    Teacher_Cadre ng(name, age, sex, 
        title, post, addr, phone, wages);
    ng.show();
